    ...
    <div className="flex items-end min-h-screen bg-[#accff8]">
      <div className="max-w-sm mx-auto px-6 py-12 relative">
        ...
        <div>
          ...
          {(activeMessage > 0) ? <Message text={`Hi there!`} /> : null}
          {(activeMessage > 1) ? <Message text={`We're from the local secondary school.`} /> : null}
          {(activeMessage > 2) ? <Message text={`You've just found one of the barn's interactive experience stations!`} /> : null}
          {(activeMessage > 3) ? <Message text={`Around the barn, you will find different stations relating to topics that affect women, girls and non-binary people.`} /> : null}
          {(activeMessage > 4) ? <Message text={`You'll be given a scenario at each station. It's your job to make decisions!`} /> : null}
        </div>
        ...
        <div className="flex justify-end">
          ...
          {(activeMessage >= 5) ? (
            <Link href="/attitudes" className="text-lg uppercase tracking-widest flex items-center justify-center h-[60px] px-6 bg-[#374590] border-5 border-[#10194a] text-white ml-3">I'm ready!</Link>
          ) : (
